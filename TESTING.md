...
编辑 `wechat-app/app.js`，确认 `apiBaseUrl` 配置：
```javascript
globalData: {
  // 小程序使用 CloudBase 云开发，无需配置本地测试服务器
  // 所有后端功能通过云函数调用
  // ...
}
```
...
1. **确保已部署云函数**
   在微信开发者工具中，右键点击云函数目录，选择「创建并部署：云端安装依赖」

2. **手机连接同一 WiFi**

3. **扫码预览**
   在微信开发者工具中点击「预览」，使用手机微信扫码即可

### 方法二：体验版（正式测试）

1. 在微信开发者工具中点击「上传」
2. 登录微信公众平台小程序后台
3. 进入「版本管理」，将上传的版本设为体验版
4. 生成体验版二维码，分享给测试人员
